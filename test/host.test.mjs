...
// ---------------------------------------------------------------------------
// usage ledger + daily budget (1.2.0)
//
// Prices are plain numbers per 1M tokens; the ledger is bucketed per local day.
// The gate must only ever skip *paid* providers — the free/local ones are what
// keep translation alive once the budget is spent.
// ---------------------------------------------------------------------------

describe('usageDayKey', function () {
  it('formats a local day as YYYY-MM-DD', function () {
    assert.equal(usageDayKey(new Date(2026, 8, 11, 12, 0, 0).getTime()), '2026-09-11')
    assert.equal(usageDayKey(new Date(2026, 0, 5, 0, 30, 0).getTime()), '2026-01-05')
  })
})

describe('hasPrice / providerCost', function () {
  it('treats only positive finite prices as paid', function () {
    assert.equal(hasPrice({ priceIn: 3 }), true)
    assert.equal(hasPrice({ priceOut: 15 }), true)
    assert.equal(hasPrice({ priceIn: 0, priceOut: 0 }), false)
    assert.equal(hasPrice({}), false)
    assert.equal(hasPrice({ priceIn: 'abc' }), false)
    assert.equal(hasPrice(null), false)
  })

  it('prices input and output tokens per 1M', function () {
    const cost = providerCost({ priceIn: 3, priceOut: 15 }, { input: 1000, output: 500 })
    // 1000/1e6*3 + 500/1e6*15 = 0.003 + 0.0075
    assert.ok(Math.abs(cost - 0.0105) < 1e-12, 'cost was ' + cost)
  })

  it('is zero without a price or without usage', function () {
    assert.equal(providerCost({}, { input: 1000, output: 1000 }), 0)
    assert.equal(providerCost({ priceIn: 3 }, undefined), 0)
    assert.equal(providerCost({ priceIn: 3 }, { input: 0, output: 0 }), 0)
  })
})

describe('budgetBlocked', function () {
  const paid = { type: 'openai', priceIn: 3 }
  const free = { type: 'google' }

  it('blocks a paid provider once today spent the limit', function () {
    const cfg = { budget: { enabled: true, dailyLimit: 1 } }
    assert.equal(budgetBlocked(cfg, paid, 1), true)
    assert.equal(budgetBlocked(cfg, paid, 0.99), false)
  })

  it('never blocks a free or local provider', function () {
    const cfg = { budget: { enabled: true, dailyLimit: 1 } }
    assert.equal(budgetBlocked(cfg, free, 999), false)
    assert.equal(budgetBlocked(cfg, { type: 'openai', apiKey: 'ollama-local' }, 999), false)
  })

  it('does nothing while the budget is off or the limit is unusable', function () {
    assert.equal(budgetBlocked({ budget: { enabled: false, dailyLimit: 1 } }, paid, 999), false)
    assert.equal(budgetBlocked({ budget: { enabled: true, dailyLimit: 0 } }, paid, 999), false)
    assert.equal(budgetBlocked({ budget: { enabled: true, dailyLimit: 'x' } }, paid, 999), false)
    assert.equal(budgetBlocked({}, paid, 999), false)
    assert.equal(budgetBlocked({ budget: { enabled: true } }, paid, 999), false)
  })
})

describe('usage ledger', function () {
  it('accumulates cost, tokens and calls per provider and day', function () {
    const ledger = emptyLedger()
    addUsageTo(ledger, '2026-09-11', 'deepseek', { input: 1000, output: 500 }, 0.0105)
    addUsageTo(ledger, '2026-09-11', 'deepseek', { input: 2000, output: 100 }, 0.0075)
    addUsageTo(ledger, '2026-09-11', 'other', { input: 10, output: 10 }, 1)
    const day = ledger.days['2026-09-11']
    assert.equal(day.providers.deepseek.calls, 2)
    assert.equal(day.providers.deepseek.input, 3000)
    assert.equal(day.providers.deepseek.output, 600)
    assert.ok(Math.abs(day.providers.deepseek.cost - 0.018) < 1e-12)
    assert.ok(Math.abs(day.total - 1.018) < 1e-12)
    assert.ok(Math.abs(totalOn(ledger, '2026-09-11') - 1.018) < 1e-12)
    assert.equal(totalOn(ledger, '2026-09-12'), 0)
    assert.equal(totalOn(emptyLedger(), '2026-09-12'), 0)
  })

  it('prunes days older than the retention window', function () {
    const now = new Date(2026, 8, 11, 12, 0, 0).getTime()
    const ledger = { days: {
      '2026-08-01': { total: 5, providers: {} },   // older than 31 days
      '2026-08-20': { total: 1, providers: {} },   // within the window
      '2026-09-11': { total: 2, providers: {} },
    } }
    pruneLedger(ledger, now)
    assert.deepEqual(Object.keys(ledger.days).sort(), ['2026-08-20', '2026-09-11'])
    assert.equal(totalOn(ledger, '2026-09-11'), 2)
  })

  it('tolerates a malformed ledger', function () {
    assert.deepEqual(pruneLedger({}, 0).days, {})
    assert.deepEqual(pruneLedger(null, 0).days, {})
  })
})

describe('usage parsing', function () {
  it('reads OpenAI-compatible and Anthropic usage shapes', function () {
    assert.deepEqual(openaiUsage({ usage: { prompt_tokens: 10, completion_tokens: 20 } }), { input: 10, output: 20 })
    assert.deepEqual(anthropicUsage({ usage: { input_tokens: 7, output_tokens: 3 } }), { input: 7, output: 3 })
  })

  it('returns undefined when the endpoint omits usage or reports zeros', function () {
    ...
    assert.equal(anthropicUsage({ usage: { input_tokens: 0, output_tokens: 0 } }), undefined)
  })

  it('accumulates across the chunks of one request', function () {
    const out = {}
    recordUsage(out, { input: 10, output: 1 })
    recordUsage(out, { input: 5, output: 2 })
    assert.deepEqual(out.usage, { input: 15, output: 3 })
    recordUsage(out, undefined)
    assert.deepEqual(out.usage, { input: 15, output: 3 })
    recordUsage(undefined, { input: 1, output: 1 }) // must not throw
  })
})

describe('pricing a read-only DSH provider', function () {
  const live = { providers: {
    gateway: { type: 'openai', source: 'dsh', apiKeyEnv: 'K', baseURL: 'https://x/v1', model: 'm' },
  } }

  it('accepts a price-only patch, because prices are local to this plugin', function () {
    const out = normalizeConfigPatch({ providers: { gateway: { priceIn: 3, priceOut: 15 } } }, live)
    assert.deepEqual(out.providers.gateway, { priceIn: 3, priceOut: 15 })
  })

  it('still refuses harness-managed fields', function () {
    const out = normalizeConfigPatch(
      { providers: { gateway: { apiKey: 'nope', baseURL: 'https://evil/v1', model: 'evil' } } },
      live,
    )
