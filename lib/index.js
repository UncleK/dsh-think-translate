...
async function viaGoogle(text, to) {
  ...
  // through the proxy; translate.google.com serves the same gtx JSON and is
  // reachable there.  The reverse may hold on other networks, so try both.
  const qs = 'translate_a/single?client=gtx&sl=auto&tl=' + encodeURIComponent(to) + '&dt=t&q=' + encodeURIComponent(text)
...
export { buildChainFromPriority, migrateConfig, normalizeConfigPatch, applyFullProviders, extractDshProviders, extractCredRefs, runChainFor, translateViaChain, removeFromChain, needsTranslate, hasProse, chunkText, fnv, deepMerge, structuredCloneSafe }
