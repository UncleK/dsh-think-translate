...
async function saveConfig(cfg) {
  ...
  for (const id of Object.keys(provs)) {
    const p = provs[id]
    if (p && p.source === 'dsh' && p.apiKey !== undefined) delete p.apiKey
  }
  await writeFile(CONFIG_PATH, JSON.stringify(diskCfg, null, 2), 'utf8')
  configCache = cfg
}
      ...
      handler: async function (req, res) {
        try {
          if (req.method === 'GET') {
            ...
            for (const id of Object.keys(provs)) {
              if (provs[id].source === 'dsh' && provs[id].apiKey !== undefined) {
                delete provs[id].apiKey
              }
            }
