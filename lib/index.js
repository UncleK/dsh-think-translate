  ...
  if (patch.providers && typeof patch.providers === 'object') {
    ...
    for (const id of Object.keys(patch.providers)) {
      ...
      if (id === 'google' || id === 'bing') {
        // Built-ins: only allow toggling enabled (type fixed).
        out.providers[id] = { enabled: p.enabled !== false }
        continue
      }
      const t = typeof p.type === 'string' ? p.type : (live.providers[id] && live.providers[id].type) || 'openai'
      if (VALID_TYPES.indexOf(t) === -1) continue
      const entry = { type: t, enabled: p.enabled !== false }
...
function applyFullProviders(live, fullMap) {
  ...
  // Preserve built-ins unless client explicitly removed them (can't remove google/bing by id since they're not in fullMap deletes — actually the client sends the complete desired map)
  for (const id of Object.keys(fullMap || {})) {
    const p = fullMap[id]
    if (!p || typeof p !== 'object') continue
    if (live.providers[id] && live.providers[id].source === 'dsh') continue
    if (id === 'google' || id === 'bing') {
      next[id] = { enabled: p.enabled !== false }
      continue
    }
