...
// Parse DSH settings.yaml to discover llm-pi-ai providers.
...
function extractDshProviders(txt) {
  const providers = {}
  // Match block starting with "llm-pi-ai:\n  providers:"
  const block = txt.match(/llm-pi-ai:\s*\n\s{2}providers:\s*\n([\s\S]*?)(?=\n\S|\n\n|\s*$)/)
  if (!block) return providers
  const body = block[1]
  // Each provider: "    name:" followed by indented fields. Use lookbehind
  // to avoid consuming the newline separator between provider blocks.
  const providerBlocks = body.match(/(?:^|(?<=\n))\s{4}[a-zA-Z0-9_-]+:\s*\n(?:\s{6}[^\n]+\n?)*/g)
  if (!providerBlocks) return providers
  for (const pb of providerBlocks) {
    const nameMatch = pb.match(/\s{4}([a-zA-Z0-9_-]+):/)
    if (!nameMatch) continue
    const name = nameMatch[1]
    const apiKeyEnv = (pb.match(/\s{6}apiKeyEnv:\s*(\S+)/) || [])[1]
    const api = (pb.match(/\s{6}api:\s*(\S+)/) || [])[1]
    const baseURL = (pb.match(/\s{6}baseURL:\s*(\S+)/) || [])[1]
    // First model id from the models list. Real settings.yaml indents
    // `models:` at 6 spaces and `- id:` at 8 spaces.
    const modelMatch = pb.match(/\s{6}models:\s*\n\s{8}- id:\s*(\S+)/)
...
// Parse .credentials.yaml refs. Returns { envName: value }.
  ...
  try {
    ...
    for (const name of Object.keys(providers)) {
      ...
      if (p.apiKeyEnv) {
        ...
        if (refs[p.apiKeyEnv]) entry.apiKey = refs[p.apiKeyEnv]
      }
      out[name] = entry
    }
...
async function mergeDshProviders(cfg) {
  ...
  for (const id of Object.keys(dsh)) {
    ...
    if (!existing) {
      ...
      cfg.providers[id] = dsh[id]
    } else if (existing.source === 'dsh') {
      // Update DSH provider (settings.yaml may have changed) but preserve
      // user's enabled/disabled state and chain membership.
      const enabled = existing.enabled
      cfg.providers[id] = Object.assign({}, dsh[id], existing, { enabled })
    }
    // If the provider already exists with a different source, keep user's version.
  }
  return cfg
}
