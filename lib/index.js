...
// - `chain` / `fallback.chain` must be arrays of known provider ids
...
function normalizeConfigPatch(patch, live) {
  ...
  if (Array.isArray(patch.chain)) {
    out.chain = patch.chain.filter(function (id) { return typeof id === 'string' && id && live.providers[id] })
  }
  if (patch.fallback && typeof patch.fallback === 'object') {
    const fbChain = Array.isArray(patch.fallback.chain)
      ? patch.fallback.chain.filter(function (id) { return typeof id === 'string' && id && live.providers[id] })
      : undefined
    out.fallback = {
  ...
  if (patch.providers && typeof patch.providers === 'object') {
    ...
    for (const id of Object.keys(patch.providers)) {
      ...
      if (live.providers[id] && live.providers[id].source === 'dsh') continue
      if (id === 'google' || id === 'bing') {
        ...
        out.providers[id] = { enabled: typeof p.enabled === 'boolean' ? p.enabled : (live.providers[id] ? live.providers[id].enabled !== false : true) }
        continue
      }
      const t = typeof p.type === 'string' ? p.type : (live.providers[id] && live.providers[id].type) || 'openai'
      ...
      const entry = {
        type: t,
        enabled: typeof p.enabled === 'boolean' ? p.enabled : (live.providers[id] ? live.providers[id].enabled !== false : false),
      }
...
// Apply a full providers-map replace (client sends `{ providers: { _full: { ...all } } }`
// after deleting entries client-side). DSH and built-in providers are preserved.
function applyFullProviders(live, fullMap) {
  const next = {}
  // Preserve DSH providers (read-only, managed by harness)
  for (const id of Object.keys(live.providers || {})) {
    const p = live.providers[id]
  ...
  for (const id of ['google', 'bing']) {
    const liveP = live.providers && live.providers[id]
    if (!liveP) continue
    const sent = fullMap && fullMap[id]
    next[id] = { enabled: sent && typeof sent.enabled === 'boolean' ? sent.enabled : liveP.enabled !== false }
  }
  for (const id of Object.keys(fullMap || {})) {
    ...
    if (live.providers[id] && live.providers[id].source === 'dsh') continue
    if (id === 'google' || id === 'bing') {
      if (typeof p.enabled === 'boolean') next[id] = { enabled: p.enabled }
      continue
    }
    const t = typeof p.type === 'string' ? p.type : 'openai'
    if (VALID_TYPES.indexOf(t) === -1) continue
    const entry = { type: t, enabled: typeof p.enabled === 'boolean' ? p.enabled : (live.providers[id] ? live.providers[id].enabled !== false : false) }
    if (typeof p.baseURL === 'string' && p.baseURL) entry.baseURL = p.baseURL
    if (typeof p.apiKey === 'string' && p.apiKey) entry.apiKey = p.apiKey
    if (typeof p.model === 'string' && p.model) entry.model = p.model
    if (typeof p.apiKeyEnv === 'string' && p.apiKeyEnv) entry.apiKeyEnv = p.apiKeyEnv
