window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function createProviderRow(id, idx, chain, provs, cfg, t, actions) {
			var p = provs[id] || {};
			var isBuiltin = id === "google" || id === "bing";
			var isDsh = p.source === "dsh";
			var label = t.labels[p.type] || p.type;
			return createElement("div", {
				...
				createElement("input", { type: "checkbox", checked: p.enabled !== false, disabled: isDsh, onChange: function (e) { actions.toggle(id, e.target.checked); } }),
		...
		function ProviderSection() {
			...
			// (Task 5); the route echoes the merged config, which we adopt so the
			// list stays in sync with what the host actually saved.
			var saveCfg = function (patch, okMsg) {
				fetch("/_xlate/config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) })
					...
					.then(function (c) { setCfg(c); if (okMsg) setMsg(okMsg); })
					.catch(function () {});
			};
			...
			// Add/edit inline form state. Fields live here (not in a nested
			// component) so re-renders of this section never reset what the
			// user is typing.
			...
			var submitForm = function () {
				...
				if (!id) return;
				var existing = cfg.providers && cfg.providers[id];
				var providers = {};
				providers[id] = {
					type: formType,
			...
			var rows = chain.map(function (id, idx) {
				return createProviderRow(id, idx, chain, provs, cfg, t, actions);
			});
			...
			if (adding || editingId) {
				formArea = createElement("div", { className: "xl-add-form" },
					...
					createElement("select", {
						className: "xl-sel", value: formType,
			...
			return createElement("div", null,
				...
				createElement("label", null,
					createElement("input", {
						...
						onChange: function (e) { saveCfg({ fallback: { enabled: e.target.checked } }); }
					}),
				...
				fbEnabled ? createElement("div", { className: "xl-fallback-chain" },
					...
					fbChain.map(function (id) {
						return createElement("span", { className: "xl-fallback-provider", key: id }, id);
					})) : null,
				modelArea,
