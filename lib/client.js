window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS_TEXT = [
			...
			".xl-form-field{display:inline-flex;align-items:center;gap:4px;min-width:0}",
			...
			".xl-suggest{display:flex;flex-wrap:wrap;gap:4px;padding:2px 0 4px 70px;max-height:150px;overflow-y:auto}",
		...
		function ProviderSection() {
			...
			var modelsForProvider = function (name) {
				...
				if (pp) return pp.models;
				// Nothing picked yet: suggest every model this endpoint type knows.
				var all = [];
				for (var i = 0; i < PROVIDER_PRESETS.length; i++) {
					if (PROVIDER_PRESETS[i].type === formType) all = all.concat(PROVIDER_PRESETS[i].models);
				}
				return all;
			};
			...
			if (adding || editingId) {
				...
				createElement("span", { className: "xl-form-field", style: { flex: "1 1 auto" } },
					...
					createElement("button", {
						...
						onClick: function () { setFormModelMenu(function (v) { return !v; }); }
					}, "▾"))),
				// Panels, not <datalist>: a datalist filters its options by what is already in
				// the input, so once a provider was picked the arrow only offered that one.
				formProviderMenu ? createElement("div", { className: "xl-suggest" },
				PROVIDER_PRESETS.map(function (pp) {
					if (pp.type !== formType) return null;
					return createElement("button", {
						key: pp.name, type: "button", className: "xl-model-item",
						onClick: function () { onProviderChange(pp.name); setFormProviderMenu(false); }
					}, pp.name);
				})) : null,
				formModelMenu ? createElement("div", { className: "xl-suggest" },
				modelsForProvider(formProvider).map(function (m) {
					return createElement("button", {
						key: m, type: "button", className: "xl-model-item",
