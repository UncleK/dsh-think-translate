window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS_TEXT = [
			...
			".xl-dsh-menu{margin:4px 0;padding:6px;border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.35));border-radius:8px;background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.06))}",
		...
		var UI_TEXT = {
			...
			"en": {
				...
				footnotes: "Footnotes",
				dshImport: "Import from DSH config",
			...
			"es": {
				...
				dshImport: "Importar desde la config de DSH",
			...
			"fr": {
				...
				dshImport: "Importer depuis la config DSH",
			...
			"de": {
				...
				footnotes: "Fußnoten",
				dshImport: "Aus DSH-Konfiguration übernehmen",
			...
			"ru": {
				...
				footnotes: "Сноски",
				dshImport: "Импортировать из конфигурации DSH",
		...
		function ProviderSection() {
			...
			var openDshImport = function () {
				fetch("/_xlate/dsh-scan").then(function (r) { return r.json(); })
					.then(function (c) {
						if (c && c.providers) { cfgRef.current = c; setCfg(c); }
						setAdding(false);
						setEditingId(null);
						setDshMenu(true);
					})
			...
			// Where the host looked for the harness settings, and what it found there:
			// an empty scan used to be a dead end, so the panel now names the file it
			// read (the usual cause is a home directory the plugin guessed wrong).
			var dshScanNote = function () {
				var d = cfg && cfg.dshScan;
				if (!d || !d.home) return null;
				var where = d.home + "/settings.yaml";
				var detail = d.settingsFound
					? (d.providers && d.providers.length ? d.providers.join(", ") : t.dshImportEmpty)
					: (t.dshImportEmpty + " · " + where);
				return createElement("div", { className: "xl-note", style: { marginTop: "4px" } }, detail);
			};
			...
			if (unused.length > 0) {
				unusedArea = createElement("div", { className: "xl-unused", style: { marginTop: "8px" } },
					createElement("div", { className: "xl-note", style: { fontWeight: 600 } }, t.providerUnused),
					unused.map(function (id) {
						...
						return createElement("div", { key: id, className: "xl-provider-row" },
							...
							isDsh ? createElement("span", { className: "xl-badge" }, t.providerDsh) : null,
							createElement("button", { className: "xl-btn", type: "button", onClick: function () { actions.addToChain(id); } }, "+ " + t.providerAddToChain));
					}));
			}
			...
			return createElement("div", null,
				...
				adding || editingId ? null
					: createElement("button", { className: "xl-btn", type: "button", onClick: startAdd, style: { marginTop: "4px" } },
						"+ " + t.providerAdd),
				adding || editingId ? null
					: createElement("button", { className: "xl-btn", type: "button", onClick: openDshImport, style: { marginTop: "4px", marginLeft: "6px" } },
						"+ " + t.dshImport + (dshAvailable.length ? " · " + dshAvailable.length : "")),
				dshMenu ? createElement("div", { className: "xl-dsh-menu" },
					dshAvailable.length === 0
						? createElement("div", null,
							createElement("div", { className: "xl-note" }, t.dshImportEmpty),
							dshScanNote())
						: createElement("div", null,
							dshAvailable.length > 1 ? createElement("button", {
								className: "xl-btn", type: "button", style: { marginBottom: "4px" }, onClick: addAllDsh
							}, t.dshImportAll) : null,
							dshAvailable.map(function (id) {
								var dp = provs[id] || {};
								return createElement("div", { key: id, className: "xl-provider-row" },
									createElement("span", { className: "xl-badge" }, t.providerDsh),
									createElement("span", { className: "xl-provider-name" }, id),
									createElement("span", { className: "xl-provider-type" }, providerLabel(id, dp, t)),
									dp.model ? createElement("span", { className: "xl-provider-type" }, dp.model) : null,
									createElement("button", {
										className: "xl-btn", type: "button",
										onClick: function () { actions.addToChain(id); }
									}, "+ " + t.providerAddToChain));
							}))) : null,
				unusedArea,
