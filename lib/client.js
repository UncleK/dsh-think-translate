	...
	factory: (require) => {
		...
		var UI_TEXT = {
			"zh-CN": {
				...
				providerApiKeyEnv: "环境变量名（可选，优先于上面的密钥）",
				footnotes: "脚注",
			...
			"en": {
				...
				providerApiKeyEnv: "Env var name (optional, overrides the key above)",
				footnotes: "Footnotes",
				...
				fallbackToggle: "Enable fallback chain",
				...
				providerManaged: "Managed by DSH settings",
				...
				modelNotInChain: "Local model (Ollama) is not in the translation chain — install/manage models below, then click 'Add to chain' to enable it",
			},
			"ja": {
				...
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				providerApiKeyEnv: "環境変数名（任意・上のキーより優先）",
				footnotes: "脚注",
			...
			"ko": {
				...
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				providerApiKeyEnv: "환경 변수 이름(선택, 위 키보다 우선)",
				footnotes: "각주",
			...
			"es": {
				...
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				providerApiKeyEnv: "Nombre de variable de entorno (opcional, prevalece sobre la clave)",
				...
				providerEnvLabel: "Variable de entorno",
				currentInUse: "En uso",
			...
			"fr": {
				...
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				providerApiKeyEnv: "Nom de variable d'environnement (facultatif, prioritaire sur la clé)",
				...
				providerEnvLabel: "Variable d'env.",
			...
			"de": {
				...
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				providerApiKeyEnv: "Name der Umgebungsvariable (optional, hat Vorrang vor dem Schlüssel)",
				footnotes: "Fußnoten",
				...
				providerEnvLabel: "Umgebungsvariable",
			...
			"ru": {
				...
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				providerApiKeyEnv: "Имя переменной окружения (необязательно, приоритет над ключом)",
				footnotes: "Сноски",
				...
				providerEnvLabel: "Переменная окр.",
		...
		function ProviderSection() {
			...
			var pickModel = function (m) {
				...
				fetch("/_xlate/config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ providers: { openai: { model: m } } }) })
					.then(function (r) { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
					.then(function (c) { cfgRef.current = c; setCfg(c); setModelMenu(null); })
					.catch(function () {});
			};
			...
			if (modelMenu === "openai") {
				...
				modelPanel = createElement("div", { className: "xl-model-panel" }, modelList);
			}
			var rows = [];
		...
		function decorateSettingsGlyph() {
			...
			for (var i = 0; i < navs.length; i++) {
				...
				for (var j = 0; j < buttons.length; j++) {
					...
					if (btn.querySelector('[data-xl-nav-glyph="1"]')) continue;
