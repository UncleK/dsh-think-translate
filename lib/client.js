window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ProviderSection() {
			...
			var modelArea = null;
			// Model download area: shown whenever the built-in openai provider
			// points at a local Ollama endpoint — independent of chain
			// membership, so the model manager stays reachable even when the
			// user removed openai from the chain (they may need to install a
			// model first, then re-add it). `priority` is legacy — chain
			// membership now drives execution.
			var openaiActive = openaiCfg.enabled !== false && Array.isArray(cfg.chain) && cfg.chain.indexOf("openai") >= 0;
			...
			if (isOllama) {
				// Hint when the local provider is configured but not in the
				// active chain: the model manager is still shown so models can
				// be installed, with a one-click re-add. Rendered above the
				// picker/pull area, not instead of it.
				var notInChainHint = null;
				if (!openaiActive) {
					notInChainHint = createElement("div", null,
						createElement("div", { className: "xl-note" }, t.modelNotInChain),
						createElement("button", { className: "xl-btn", type: "button", onClick: function () { actions.addToChain("openai"); }, style: { marginBottom: "6px" } },
							"+ " + t.providerAddToChain));
				}
				if (models === null) {
					modelArea = createElement("div", null,
						notInChainHint,
						createElement("div", { className: "xl-note" }, t.loadingModels));
				} else {
					...
					var picker = null;
					if (models.length > 0) {
						picker = createElement("div", { className: "xl-model-row" },
							createElement("label", { style: { flex: "0 1 auto", display: "flex", gap: "6px", alignItems: "center" } },
								createElement("span", { style: { flex: "0 0 auto" } }, t.model),
							...
							createElement("button", {
								className: "xl-btn", type: "button", title: t.downloadModel,
								style: { padding: "2px 8px", lineHeight: 1, fontSize: "12px" },
					...
					modelArea = createElement("div", null, notInChainHint, picker, pullArea);
				}
			}
			// ---- dynamic provider chain UI ---------------------------------
			// (inline form state is declared above, before the cfg===null guard)
			var actions = {
			...
			var rows = chain.map(function (id) {
				return createProviderRow(id, chain, provs, cfg, t, actions);
			});
