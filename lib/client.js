window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS_TEXT = [
			...
			".xl-fallback-chain{display:flex;flex-wrap:wrap;gap:6px;align-items:center;font-size:12px;margin:4px 0}",
			".xl-fallback-provider{padding:2px 8px;border-radius:4px;background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.12));font-size:11px;opacity:.8}",
		...
		function ProviderSection() {
			...
			var formNamePair = useDraft("formName", ""); var formName = formNamePair[0], setFormName = formNamePair[1];
			...
			var applyLocal = function (patch) {
				...
				if (patch.fallback && typeof patch.fallback === "object") {
					next.fallback = Object.assign({}, cur.fallback || {}, patch.fallback);
				}
				cfgRef.current = next;
				setCfg(next);
			};
			...
			if (adding || editingId) {
				...
				createElement("label", { className: "xl-form-row" },
					...
					// The env name has no row of its own (it arrives from a preset or a
					// DSH row), so show it here: without the badge the value is invisible
					// and typing a key would look like it had no effect.
					formApiKeyEnv.trim() ? createElement("span", { className: "xl-badge", title: t.providerApiKey }, "env:" + formApiKeyEnv.trim()) : null,
					createElement("input", {
						className: "xl-cfg", placeholder: t.providerApiKey, value: formApiKey,
						onChange: function (e) {
							var v = e.target.value;
							setFormApiKey(v);
							// Typing a key means "use this key": drop the env name, or the
							// host would keep preferring the environment value.
