window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var MODEL_PRESETS = [
			{ type: "openai", label: "DeepSeek · deepseek-chat", baseURL: "https://api.deepseek.com/v1", model: "deepseek-chat", env: "DEEPSEEK_API_KEY" },
			{ type: "openai", label: "OpenAI · gpt-4o-mini", baseURL: "https://api.openai.com/v1", model: "gpt-4o-mini", env: "OPENAI_API_KEY" },
			{ type: "openai", label: "阿里云百炼 · qwen-turbo", baseURL: "https://dashscope.aliyuncs.com/compatible-mode/v1", model: "qwen-turbo", env: "DASHSCOPE_API_KEY" },
			{ type: "openai", label: "智谱 · glm-4-flash", baseURL: "https://open.bigmodel.cn/api/paas/v4", model: "glm-4-flash", env: "ZHIPUAI_API_KEY" },
			{ type: "openai", label: "月之暗面 · moonshot-v1-8k", baseURL: "https://api.moonshot.cn/v1", model: "moonshot-v1-8k", env: "MOONSHOT_API_KEY" },
			{ type: "openai", label: "硅基流动 · Qwen2.5-7B-Instruct", baseURL: "https://api.siliconflow.cn/v1", model: "Qwen/Qwen2.5-7B-Instruct", env: "SILICONFLOW_API_KEY" },
			{ type: "anthropic", label: "Anthropic · claude-3-5-haiku-latest", baseURL: "https://api.anthropic.com", model: "claude-3-5-haiku-latest", env: "ANTHROPIC_API_KEY" },
			{ type: "anthropic", label: "Anthropic · claude-sonnet-4-5", baseURL: "https://api.anthropic.com", model: "claude-sonnet-4-5", env: "ANTHROPIC_API_KEY" },
		];
		var UI_TEXT = {
			"zh-CN": {
		...
		function ProviderSection() {
			...
			var formPresetPair = useState(""); var formPreset = formPresetPair[0], setFormPreset = formPresetPair[1];
			...
			// Fill the form from a preset; the user can still edit every field.
			var applyPreset = function (index) {
				var pp = MODEL_PRESETS[index];
				if (!pp) { setFormPreset(""); return; }
				setFormPreset(String(index));
				setFormType(pp.type);
				setFormBaseURL(pp.baseURL);
				setFormModel(pp.model);
				setFormApiKeyEnv(pp.env);
			};
			// Typing a model name detaches the form from the preset it came from, so
			// the provider select never keeps claiming a provider the model no longer
			// matches.
			var onModelChange = function (value) {
				setFormModel(value);
				var pp = formPreset === "" ? null : MODEL_PRESETS[Number(formPreset)];
				if (pp && value !== pp.model) setFormPreset("");
			};
			...
			if (adding || editingId) {
				...
				createElement("label", { className: "xl-form-row" },
					...
					createElement("select", {
						...
						onChange: function (e) { setFormType(e.target.value); setFormPreset(""); }
					},
				...
				createElement("label", { className: "xl-form-row" },
					...
					createElement("select", {
						className: "xl-sel", style: { flex: "0 0 148px" }, value: formPreset,
						onChange: function (e) { applyPreset(e.target.value); }
					},
						createElement("option", { value: "" }, t.presetCustom),
						MODEL_PRESETS.map(function (pp, i) {
							if (pp.type !== formType) return null;
							return createElement("option", { key: pp.model, value: String(i) }, pp.label);
						})),
					createElement("input", {
						className: "xl-cfg", style: { flex: "1 1 auto" }, placeholder: t.providerModel, value: formModel,
				...
				createElement("label", { className: "xl-form-row" },
					createElement("span", { className: "xl-form-label" }, t.providerEnvLabel),
					createElement("input", {
						className: "xl-cfg", placeholder: "DEEPSEEK_API_KEY", value: formApiKeyEnv,
						onChange: function (e) { setFormApiKeyEnv(e.target.value); }
					})),
				createElement("div", { style: { display: "flex", gap: "6px" } },
