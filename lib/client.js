window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS_TEXT = [
			...
			".xl-dock{box-sizing:border-box;flex:none;overflow:hidden;margin:0 auto 4px;width:100%;border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.35));border-radius:12px;background:var(--dsw-specific-tip,var(--dsw-alias-bg-layer-2,rgba(127,127,127,.08)))}",
			".xl-dock-body{display:flex;flex-direction:column;gap:8px;padding:6px 12px}",
			".xl-dock-header{display:flex;align-items:center;gap:10px;width:100%;padding:0;border:none;background:transparent;text-align:left;cursor:pointer;font:inherit;color:inherit}",
			".xl-dock-title{flex:none;font-size:13px;line-height:24px;font-weight:500;color:var(--dsw-alias-label-primary,inherit)}",
			".xl-dock-progress{flex:1 1 auto;min-width:0;overflow:hidden;font-size:13px;line-height:20px;color:var(--dsw-alias-label-tertiary,rgba(127,127,127,.75));text-overflow:ellipsis;white-space:nowrap}",
			".xl-dock-chev{display:grid;flex:none;place-items:center;font-size:11px;color:var(--dsw-alias-label-tertiary,rgba(127,127,127,.75))}",
			...
			".xl-dock-item{display:flex;align-items:center;gap:10px;min-width:0;font-size:13px;line-height:20px;color:var(--dsw-alias-label-secondary,inherit)}",
			".xl-dock-glyph{display:grid;flex:none;place-items:center;width:16px;height:16px;font-size:12px;opacity:.7}",
			".xl-dock-content{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;opacity:.85}"
		].join("\n");
		...
		function XDockTodoItem(props) {
			...
			var hasTr = !!(tr && tr.ok && !tr.skipped && tr.text !== content);
			var glyph = props.item.status === "completed" ? "✓" : props.item.status === "in_progress" ? "◐" : "○";
			return createElement("li", { className: "xl-dock-item", "data-status": props.item.status },
			...
			return createElement("section", { className: "xl-dock", "data-testid": "todo-panel", "aria-label": title },
				createElement("div", { className: "xl-dock-body" },
					createElement("button", {
						...
						createElement("span", { className: "xl-dock-chev", "aria-hidden": true }, open ? "▾" : "▸")),
					open ? createElement("ul", { className: "xl-dock-list" }, list.map(function (it, n) {
						return createElement(XDockTodoItem, { key: n, item: it });
					})) : null));
		}
