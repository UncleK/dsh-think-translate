window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Settings nav glyph (DSH >= 0.1.5 exposes the seat). The shell's own table
		// is per-id with the settings gear for every plugin section, so without this
		// "思考链翻译" looks like the generic settings entry.
		var IconThink = primitives && primitives.IconThinkOutline16 ? primitives.IconThinkOutline16 : null;
		// Shown at the foot of the settings panel. The host's /_xlate/version route is
		// the source of truth; these constants cover the case where that route is not
		// there yet (an older process still serving the port), and the contract test
		// pins PLUGIN_VERSION to package.json so the two can never drift.
		var PLUGIN_NAME = "dsh-think-translate";
		var PLUGIN_VERSION = "1.2.2";
		var PLUGIN_REPO = "https://github.com/UncleK/dsh-think-translate";
		// The official MarkdownText contract changed in 0.1.2-alpha: it now takes a
		// nested `labels` object and dereferences it without defaults inside its code
		// fence and footnote renderers (context.labels.code.copyLabel), so a message with
		// a fenced block threw when only the old flat `codeLabels` prop was passed
		// (issue #3). Build the nested shape, and hand its code half to the old prop too,
		// so one build serves both the 0.1.1-rc and the 0.1.2+ platforms. The objects are
		// cached per UI_TEXT dictionary because MarkdownText is memoized: a fresh identity
		// on every render would silently defeat that memo.
		var mdLabelsCache = typeof WeakMap === "function" ? new WeakMap() : null;
		function markdownLabels(tt) {
			if (!tt) return undefined;
			if (mdLabelsCache) {
				var cached = mdLabelsCache.get(tt);
				if (cached) return cached;
			}
			var labels = {
				code: { copyLabel: tt.copy, copiedLabel: tt.copied },
				footnotes: tt.footnotes || "Footnotes",
			};
			if (mdLabelsCache) mdLabelsCache.set(tt, labels);
			return labels;
		}
		...
		var CSS_TEXT = [
			...
			".xl-panel label > span:first-child{flex:0 0 70px;font-size:12.5px;opacity:.85}",
			...
			".xl-provider-row{display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.15));cursor:grab}",
			".xl-provider-row:active{opacity:.55}",
			".xl-provider-type{font-size:11px;opacity:.6;flex:0 0 auto}",
			".xl-provider-name{font-size:12px;font-weight:500;flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			".xl-badge{font-size:10px;line-height:16px;padding:0 6px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2,rgba(127,127,127,.45));background:transparent;color:inherit;opacity:.7;font-weight:500;white-space:nowrap}",
			".xl-add-form{display:flex;flex-direction:column;gap:6px;padding:8px;background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.08));border-radius:8px;margin:4px 0}",
			".xl-form-row{display:flex;align-items:center;gap:8px}",
			".xl-form-label{flex:0 0 62px;font-size:11.5px;opacity:.65}",
			".xl-form-field{position:relative;display:inline-flex;align-items:center;gap:4px;min-width:0}",
			".xl-form-field .xl-cfg{flex:1 1 auto;min-width:0;width:auto}",
			".xl-caret{border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.35));background:transparent;color:inherit;font-size:10px;line-height:1;border-radius:6px;padding:4px 6px;cursor:pointer}",
			".xl-caret:hover{background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.12))}",
			".xl-menu{position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:60;max-height:190px;overflow-y:auto;padding:2px;border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.4));border-radius:8px;background:var(--dsw-alias-bg-overlay,var(--dsw-alias-bg-base,#fff));box-shadow:0 8px 24px rgba(0,0,0,.18)}",
			".xl-menu-item{display:block;width:100%;text-align:left;border:0;background:transparent;color:inherit;font-size:12px;line-height:1.6;padding:4px 8px;border-radius:6px;cursor:pointer;font-family:var(--dsw-font-mono,monospace)}",
			".xl-menu-item:hover{background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.14))}",
			".xl-menu-item-active{color:var(--dsw-alias-brand-primary,#4c9aff);font-weight:600}",
			".xl-menu-note{font-size:11px;opacity:.55;padding:4px 8px 2px;margin-top:2px;border-top:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.2))}",
			".xl-caret:disabled{opacity:.35;cursor:default}",
			".xl-unused{opacity:.85}",
			".xl-kv-value{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			".xl-kv-sep{opacity:.3;padding:0 1px}",
			".xl-about{display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px;opacity:.75}",
			".xl-chip{border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.35));background:transparent;color:inherit;font-size:11px;border-radius:999px;padding:1px 8px;cursor:pointer;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			".xl-chip:hover{border-color:var(--dsw-alias-border-l2,rgba(127,127,127,.7));background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.1))}",
			".xl-model-panel{margin:2px 0 8px 12px;padding:8px;border-left:2px solid rgba(127,127,127,.3);display:flex;flex-direction:column;gap:6px}",
			".xl-model-menu{display:flex;flex-wrap:wrap;gap:4px;max-height:180px;overflow-y:auto}",
			".xl-model-item{border:1px solid var(--dsw-alias-border-l1,rgba(127,127,127,.35));background:transparent;color:inherit;font-size:11.5px;border-radius:6px;padding:3px 8px;cursor:pointer;font-family:var(--dsw-font-mono,monospace)}",
			".xl-model-item:hover{background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.12))}",
			".xl-model-item-active{border-color:var(--dsw-alias-brand-primary,#4c9aff);color:var(--dsw-alias-brand-primary,#4c9aff)}",
			".xl-x-btn{border:0;background:none;color:inherit;opacity:.5;cursor:pointer;font-size:15px;line-height:1;padding:0 6px;border-radius:4px}",
			".xl-x-btn:hover{opacity:1;background:var(--dsw-alias-bg-layer-2,rgba(127,127,127,.12))}",
			".xl-add-btn{padding:1px 7px;font-size:14px;line-height:1.35;font-weight:600}",
			".xl-provider-row .xl-sel{max-width:260px}",
			".xl-model-row{display:flex;align-items:center;gap:6px;flex-wrap:wrap}",
			".xl-model-row .xl-sel{max-width:220px}",
			".xl-settings-form label > span:first-child{flex:0 0 auto}",
		].join("\n");
		...
		// Starting points for the custom-provider form: the cheap, high-value
		// endpoints people actually translate with. Every field stays editable, so
		// a preset that drifts out of date costs nothing but a retype.
		// Known endpoints, grouped by provider: the form suggests provider NAMES, and
		// once one is picked the model field offers that provider's usual models (the
		// cheap, translation-worthy ones first). Both fields stay free text — a private
		// gateway or a brand-new model is never blocked here.
		//
		// The model ids were checked against the vendors' own documentation on
		// 2026-09-11, and they do drift: Qwen retired qwen-turbo in July 2026, Moonshot
		// retired the whole moonshot-v1 series in August, and the Claude 3.x ids are
		// gone. That is why both menus end with "suggestions only" and the fields stay
		// editable — treat the list as a convenience, not a contract.
		var PROVIDER_PRESETS = [
		{ type: "openai", name: "DeepSeek", baseURL: "https://api.deepseek.com/v1", env: "DEEPSEEK_API_KEY", models: ["deepseek-flash", "deepseek-v4-pro"] },
		{ type: "openai", name: "OpenAI", baseURL: "https://api.openai.com/v1", env: "OPENAI_API_KEY", models: ["gpt-5.4-nano", "gpt-5.6-luna", "gpt-5.4-mini", "gpt-5.6-terra", "gpt-6-astra"] },
		{ type: "openai", name: "阿里云百炼", baseURL: "https://dashscope.aliyuncs.com/compatible-mode/v1", env: "DASHSCOPE_API_KEY", models: ["qwen3.5-flash", "qwen3.6-flash", "qwen3.7-plus", "qwen3.7-max"] },
		{ type: "openai", name: "智谱 GLM", baseURL: "https://open.bigmodel.cn/api/paas/v4", env: "ZHIPUAI_API_KEY", models: ["glm-5.3-flash", "glm-5.2", "glm-5.3"] },
		{ type: "openai", name: "月之暗面 Kimi", baseURL: "https://api.moonshot.cn/v1", env: "MOONSHOT_API_KEY", models: ["kimi-k2.6", "kimi-k3", "kimi-k2.7-code", "kimi-k2.7-code-highspeed"] },
		{ type: "openai", name: "硅基流动", baseURL: "https://api.siliconflow.cn/v1", env: "SILICONFLOW_API_KEY", models: ["Pro/deepseek-ai/DeepSeek-R1"] },
		{ type: "openai", name: "OpenRouter", baseURL: "https://openrouter.ai/api/v1", env: "OPENROUTER_API_KEY", models: ["openrouter/auto"] },
		{ type: "anthropic", name: "Anthropic", baseURL: "https://api.anthropic.com", env: "ANTHROPIC_API_KEY", models: ["claude-haiku-4-5-20251001", "claude-sonnet-5", "claude-opus-5", "claude-fable-5-1"] },
		];
		var UI_TEXT = {
			"zh-CN": {
				...
				loadingModels: "加载模型…",
				testing: "测试中…", connOk: "连通", connFail: "链路失败（将回退浏览器直连）",
				clearCache: "清空缓存", status: "成功/失败", cacheEntries: "缓存条目", lastError: "最近错误",
				...
				copy: "复制", copied: "已复制",
				providerAdd: "添加自定义提供方",
				providerType: "类型",
				providerBaseURL: "接口地址",
				providerApiKey: "API 密钥",
				footnotes: "脚注",
				dshImport: "重新扫描 DSH 配置",
				dshImportAll: "全部加入链",
				dshImportEmpty: "DSH 里没有发现已配置的 provider",
				dshRescanning: "正在重新扫描 DSH 配置…",
				dshRescanDone: "重新扫描完成",
				providerDshList: "DSH 已配置提供方",
				starCta: "给个星星鼓励一下",
				providerPreset: "提供方",
				currentInUse: "当前使用",
				chooseModel: "选择模型",
				...
				providerDelete: "删除",
				providerDsh: "DSH",
				providerHint: "仅为建议，可直接填写其他提供方",
				modelHint: "仅为建议，可直接输入模型名",
				providerUp: "上移",
				providerDown: "下移",
				providerOn: "开启",
				providerOff: "关闭",
				providerSaved: "已保存",
				providerDeleted: "已删除",
				providerDrag: "翻译优先级（拖动排序）",
				providerCancel: "取消",
				providerSave: "保存",
				providerSaveError: "保存失败",
				providerOpenAICompat: "OpenAI 兼容",
				providerUnused: "可用提供方（未在链中）",
				providerAddToChain: "加入链",
				providerRemoveFromChain: "移出链",
			},
			"en": {
				...
				testing: "Testing…", connOk: "Connected", connFail: "Chain failed (will fall back)",
				clearCache: "Clear cache", status: "ok / fail", cacheEntries: "cache entries", lastError: "last error",
				...
				providerAdd: "Add custom provider",
				providerType: "Type",
				providerBaseURL: "Base URL",
				providerApiKey: "API key",
				footnotes: "Footnotes",
				dshImport: "Rescan DSH config",
				dshImportAll: "Add all to chain",
				dshImportEmpty: "No provider configured in DSH",
				dshRescanning: "Rescanning the DSH config…",
				dshRescanDone: "Rescan complete",
				providerDshList: "Configured in DSH",
				...
				providerDsh: "DSH",
				providerHint: "Suggestions only — type any provider",
				modelHint: "Suggestions only — type any model id",
				providerUp: "up",
				providerDown: "down",
				providerOn: "on",
				providerOff: "off",
				...
				providerDrag: "Translation priority (drag to reorder)",
				providerCancel: "Cancel",
				providerSave: "Save",
				providerSaveError: "Save failed",
				providerOpenAICompat: "OpenAI-compatible",
				providerUnused: "Available providers (not in chain)",
				providerAddToChain: "Add to chain",
				providerRemoveFromChain: "Remove from chain",
			},
			"ja": {
				...
				testing: "テスト中…", connOk: "接続OK", connFail: "リンク失敗（フォールバック）",
				clearCache: "キャッシュをクリア", status: "成功/失敗", cacheEntries: "キャッシュ件数", lastError: "最近のエラー",
				...
				providerAdd: "カスタム提供元を追加",
				providerType: "種類",
				providerBaseURL: "ベース URL",
				providerApiKey: "API キー",
				copy: "コピー", copied: "コピー済み",
				footnotes: "脚注",
				dshImport: "DSH 設定を再スキャン",
				dshImportAll: "すべてチェーンに追加",
				dshImportEmpty: "DSH に設定済みプロバイダーがありません",
				dshRescanning: "DSH 設定を再スキャン中…",
				dshRescanDone: "再スキャン完了",
				providerDshList: "DSH に設定済みの提供元",
				starCta: "GitHub でスターを",
				providerPreset: "提供元",
				currentInUse: "使用中",
				chooseModel: "モデルを選択",
				presetCustom: "自分で入力",
				...
				providerDelete: "削除",
				providerDsh: "DSH",
				providerHint: "候補のみ・自由に入力できます",
				modelHint: "候補のみ・モデル名を直接入力できます",
				providerUp: "上へ",
				providerDown: "下へ",
				providerOn: "オン",
				providerOff: "オフ",
				providerSaved: "保存しました",
				providerDeleted: "削除しました",
				providerDrag: "翻訳の優先順位（ドラッグで並べ替え）",
				providerCancel: "キャンセル",
				providerSave: "保存",
				providerSaveError: "保存に失敗",
				providerOpenAICompat: "OpenAI 互換",
				providerUnused: "利用可能な提供元（チェーン未参加）",
				providerAddToChain: "チェーンに追加",
				providerRemoveFromChain: "チェーンから外す",
			},
			"ko": {
				...
				testing: "테스트 중…", connOk: "연결됨", connFail: "체인 실패（폴백）",
				clearCache: "캐시 지우기", status: "성공/실패", cacheEntries: "캐시 항목", lastError: "최근 오류",
				...
				providerAdd: "사용자 지정 제공자 추가",
				providerType: "유형",
				providerBaseURL: "베이스 URL",
				providerApiKey: "API 키",
				copy: "복사", copied: "복사됨",
				footnotes: "각주",
				dshImport: "DSH 설정 다시 검사",
				dshImportAll: "모두 체인에 추가",
				dshImportEmpty: "DSH에 구성된 제공자가 없습니다",
				dshRescanning: "DSH 설정을 다시 검사하는 중…",
				dshRescanDone: "다시 검사 완료",
				providerDshList: "DSH에 설정된 제공자",
				starCta: "GitHub에서 별 주기",
				providerPreset: "제공자",
				currentInUse: "사용 중",
				chooseModel: "모델 선택",
				presetCustom: "직접 입력",
				...
				providerDelete: "삭제",
				providerDsh: "DSH",
				providerHint: "추천 목록일 뿐입니다 · 직접 입력 가능",
				modelHint: "추천 목록일 뿐입니다 · 모델명 직접 입력 가능",
				providerUp: "위로",
				providerDown: "아래로",
				providerOn: "켜기",
				providerOff: "끄기",
				providerSaved: "저장됨",
				providerDeleted: "삭제됨",
				providerDrag: "번역 우선순위(끌어서 정렬)",
				providerCancel: "취소",
				providerSave: "저장",
				providerSaveError: "저장 실패",
				providerOpenAICompat: "OpenAI 호환",
				providerUnused: "사용 가능한 제공자(체인 미포함)",
				providerAddToChain: "체인에 추가",
				providerRemoveFromChain: "체인에서 제거",
			},
			"es": {
				...
				testing: "Probando…", connOk: "Conectado", connFail: "Fallo de cadena (usará respaldo)",
				clearCache: "Limpiar caché", status: "éxito / fallo", cacheEntries: "entradas de caché", lastError: "último error",
				...
				providerAdd: "Añadir proveedor personalizado",
				providerType: "Tipo",
				providerBaseURL: "URL base",
				providerApiKey: "Clave de API",
				copy: "Copiar", copied: "Copiado",
				footnotes: "Notas al pie",
				dshImport: "Reescanear la config de DSH",
				dshImportAll: "Añadir todo a la cadena",
				dshImportEmpty: "No hay proveedores configurados en DSH",
				dshRescanning: "Reescaneando la config de DSH…",
				dshRescanDone: "Reescaneo completado",
				providerDshList: "Configurados en DSH",
				starCta: "Danos una estrella",
				...
				providerTest: "Probar",
				providerEdit: "Editar",
				providerDelete: "Eliminar",
				providerDsh: "DSH",
				providerHint: "Solo sugerencias: escribe el que quieras",
				modelHint: "Solo sugerencias: escribe cualquier modelo",
				providerUp: "Subir",
				providerDown: "Bajar",
				providerOn: "Activado",
				providerOff: "Desactivado",
				providerSaved: "Guardado",
				providerDeleted: "Eliminado",
				providerDrag: "Prioridad de traducción (arrastra para reordenar)",
				providerCancel: "Cancelar",
				providerSave: "Guardar",
				providerSaveError: "Error al guardar",
				providerOpenAICompat: "Compatible con OpenAI",
				providerUnused: "Proveedores disponibles (fuera de la cadena)",
				providerAddToChain: "Añadir a la cadena",
				providerRemoveFromChain: "Quitar de la cadena",
			},
			"fr": {
				...
				testing: "Test en cours…", connOk: "Connecté", connFail: "Échec de chaîne (repli)",
				clearCache: "Vider le cache", status: "ok / échec", cacheEntries: "entrées de cache", lastError: "dernière erreur",
				...
				providerAdd: "Ajouter un fournisseur personnalisé",
				providerType: "Type",
				providerBaseURL: "URL de base",
				providerApiKey: "Clé API",
				copy: "Copier", copied: "Copié",
				footnotes: "Notes de bas de page",
				dshImport: "Réanalyser la config DSH",
				dshImportAll: "Tout ajouter à la chaîne",
				dshImportEmpty: "Aucun fournisseur configuré dans DSH",
				dshRescanning: "Réanalyse de la config DSH…",
				dshRescanDone: "Réanalyse terminée",
				providerDshList: "Configurés dans DSH",
				starCta: "Mettre une étoile",
				providerPreset: "Fournisseur",
				currentInUse: "En cours d'utilisation",
				chooseModel: "Choisir un modèle",
				presetCustom: "Personnalisé",
				providerModel: "Modèle",
				providerTest: "Tester",
				providerEdit: "Modifier",
				providerDelete: "Supprimer",
				providerDsh: "DSH",
				providerHint: "Suggestions seulement : saisissez le vôtre",
				modelHint: "Suggestions seulement : saisissez un modèle",
				providerUp: "Monter",
				providerDown: "Descendre",
				providerOn: "Activé",
				providerOff: "Désactivé",
				providerSaved: "Enregistré",
				providerDeleted: "Supprimé",
				providerDrag: "Priorité de traduction (glisser pour réordonner)",
				providerCancel: "Annuler",
				providerSave: "Enregistrer",
				providerSaveError: "Échec de l'enregistrement",
				providerOpenAICompat: "Compatible OpenAI",
				providerUnused: "Fournisseurs disponibles (hors chaîne)",
				providerAddToChain: "Ajouter à la chaîne",
				providerRemoveFromChain: "Retirer de la chaîne",
			},
			"de": {
				...
				testing: "Teste…", connOk: "Verbunden", connFail: "Kette fehlgeschlagen (Fallback)",
				clearCache: "Cache leeren", status: "ok / fehlgeschlagen", cacheEntries: "Cache-Einträge", lastError: "letzter Fehler",
				...
				providerAdd: "Eigenen Anbieter hinzufügen",
				providerType: "Typ",
				providerBaseURL: "Basis-URL",
				providerApiKey: "API-Schlüssel",
				copy: "Kopieren", copied: "Kopiert",
				footnotes: "Fußnoten",
				dshImport: "DSH-Konfiguration neu einlesen",
				dshImportAll: "Alle zur Kette hinzufügen",
				dshImportEmpty: "Kein konfigurierter Anbieter in DSH",
				dshRescanning: "DSH-Konfiguration wird neu eingelesen…",
				dshRescanDone: "Neu eingelesen",
				providerDshList: "In DSH konfiguriert",
				starCta: "Mit einem Stern danken",
				providerPreset: "Anbieter",
				currentInUse: "In Benutzung",
				chooseModel: "Modell wählen",
				presetCustom: "Selbst ausfüllen",
				providerModel: "Modell",
				providerTest: "Testen",
				providerEdit: "Bearbeiten",
				providerDelete: "Löschen",
				providerDsh: "DSH",
				providerHint: "Nur Vorschläge – eigener Name möglich",
				modelHint: "Nur Vorschläge – Modell frei eingeben",
				providerUp: "Nach oben",
				providerDown: "Nach unten",
				providerOn: "An",
				providerOff: "Aus",
				providerSaved: "Gespeichert",
				providerDeleted: "Gelöscht",
				providerDrag: "Übersetzungspriorität (zum Sortieren ziehen)",
				providerCancel: "Abbrechen",
				providerSave: "Speichern",
				providerSaveError: "Speichern fehlgeschlagen",
				providerOpenAICompat: "OpenAI-kompatibel",
				providerUnused: "Verfügbare Anbieter (nicht in der Kette)",
				providerAddToChain: "Zur Kette hinzufügen",
				providerRemoveFromChain: "Aus der Kette entfernen",
			},
			"ru": {
				...
				testing: "Проверка…", connOk: "Подключено", connFail: "Сбой цепочки (запасной вариант)",
				clearCache: "Очистить кэш", status: "ок / сбой", cacheEntries: "записи кэша", lastError: "последняя ошибка",
				...
				providerAdd: "Добавить свой провайдер",
				providerType: "Тип",
				providerBaseURL: "Базовый URL",
				providerApiKey: "Ключ API",
				copy: "Копировать", copied: "Скопировано",
				footnotes: "Сноски",
				dshImport: "Пересканировать конфиг DSH",
				dshImportAll: "Добавить все в цепочку",
				dshImportEmpty: "В DSH нет настроенных провайдеров",
				dshRescanning: "Пересканирую конфиг DSH…",
				dshRescanDone: "Сканирование завершено",
				providerDshList: "Настроены в DSH",
				starCta: "Поставить звезду",
				providerPreset: "Провайдер",
				currentInUse: "Используется",
				chooseModel: "Выбрать модель",
				presetCustom: "Свой",
				providerModel: "Модель",
				providerTest: "Проверить",
				providerEdit: "Изменить",
				providerDelete: "Удалить",
				providerDsh: "DSH",
				providerHint: "Только подсказки — можно ввести свой",
				modelHint: "Только подсказки — введите свой ID",
				providerUp: "Вверх",
				providerDown: "Вниз",
				providerOn: "Вкл",
				providerOff: "Выкл",
				providerSaved: "Сохранено",
				providerDeleted: "Удалено",
				providerDrag: "Приоритет перевода (перетащите для порядка)",
				providerCancel: "Отмена",
				providerSave: "Сохранить",
				providerSaveError: "Не удалось сохранить",
				providerOpenAICompat: "Совместимо с OpenAI",
				providerUnused: "Доступные провайдеры (не в цепочке)",
				providerAddToChain: "Добавить в цепочку",
				providerRemoveFromChain: "Убрать из цепочки",
			},
		};
		...
		// The persisted cache is bounded on purpose: one localStorage key per
		// translated text would otherwise grow forever, and once the origin quota
		// is full every OTHER localStorage writer in this app fails silently too —
		// including this plugin's own settings. Entries carry `at` so the prune
		// can drop the oldest ones first (2026-09-11).
		var LS_CACHE_MAX = 300;
		var cacheWrites = 0;
		function pruneCache() {
			try {
				var keys = [];
				...
				if (keys.length <= LS_CACHE_MAX) return;
				var rows = keys.map(function (k) {
					var at = 0;
					try {
						var v = JSON.parse(localStorage.getItem(k) || "null");
						if (v && typeof v.at === "number") at = v.at;
					} catch (e) {}
					return { k: k, at: at };
				});
				rows.sort(function (a, b) { return a.at - b.at; });
				for (var j = 0; j < rows.length - LS_CACHE_MAX; j++) localStorage.removeItem(rows[j].k);
			} catch (e) {}
		}
		function cacheMemSet(key, entry) {
			...
			try {
				var stamped = entry && typeof entry === "object" ? Object.assign({ at: Date.now() }, entry) : entry;
				localStorage.setItem(MEM_KEY + key, JSON.stringify(stamped));
				cacheWrites++;
				if (cacheWrites % 20 === 0) pruneCache();
			} catch (e) {}
		}
		var pend = new Map();
		...
		async function gtxChunk(text, target) {
			var qs = "translate_a/single?client=dict-chrome-ex&sl=auto&tl="
				+ encodeURIComponent(target) + "&dt=t&q=" + encodeURIComponent(text);
			// translate.googleapis.com is TLS-reset by Clash/GFW in CN networks;
			// translate.google.com serves the same gtx JSON and is reachable there.
			// Try both so CN and the rest of the world both work.
			var hosts = ["https://translate.google.com/", "https://translate.googleapis.com/"];
			var lastErr = null;
			...
			throw lastErr || new Error("gtx unavailable");
		}
		...
		class XBoundary extends Component {
			...
			componentDidCatch(err) {
				// The boundary only surfaces the message inline; log the full
				// error so the next crash is pinpointable from the browser
				// console (2026-09-03).
				try {
					if (typeof console !== "undefined" && console.error) {
						console.error("[xlate] render error:", err && err.stack ? err.stack : err);
					}
				} catch (e) {}
			}
			render() {
				if (this.state.err) {
					...
					if (fb !== null && fb !== undefined) {
						// Caller-supplied original-content fallback: a crashed
						// block must show its original text, never blank the
						// answer (2026-09-03: a 0.1.2-alpha renderer threw while
						// composing an answer and the whole answer was replaced
						// by the bare "[xlate] ..." line).
						return createElement("div", null,
							createElement("div", { className: "xl-warn" }, "[xlate] " + String(this.state.err && this.state.err.message ? this.state.err.message : this.state.err)),
							fb);
					}
		...
		function TextBlock(props) {
			...
			var mdLabels = MarkdownText ? markdownLabels(tt) : undefined;
			// Flat `codeLabels` is what ≤ 0.1.1-rc renderers read; the 0.1.2+ ones ignore
			// it and read the nested `labels` instead.
			var codeLabels = mdLabels ? mdLabels.code : undefined;
			var MDT = MarkdownText || Markdownish;
			var body = hasTr ? tr.text : props.text;
			return createElement("div", null,
				createElement(MDT, MarkdownText
					? { text: body, streaming: props.streaming, labels: mdLabels, codeLabels: codeLabels }
				...
				hasTr ? createElement("details", { className: "xl-orig" },
					...
					createElement("div", null, createElement(MDT, MarkdownText
						? { text: props.text, labels: mdLabels, codeLabels: codeLabels }
		...
		function AssistantView(props) {
			...
			for (var i = 0; i < blocks.length; i++) {
				...
				if (!block) continue;
				if (block.kind === "text") {
					rendered.push(createElement(TextBlockSafe, { key: "t" + i, text: block.text, streaming: streaming }));
		...
		function originalBlocksText(blocks) {
			var parts = [];
			for (var i = 0; i < blocks.length; i++) {
				var b = blocks[i];
				if (!b) continue;
				if ((b.kind === "text" || b.kind === "reasoning") && typeof b.text === "string" && b.text.trim()) parts.push(b.text);
			}
			return parts;
		}
		// Per-text-block fuse: the official MarkdownText path stays the normal
		// renderer for streaming and settled text alike. If it throws for a
		// specific block, only that block degrades to its ORIGINAL text through
		// the crash-proof Markdownish renderer (plus a one-line warning) instead
		// of the whole answer being replaced by the [xlate] error line.
		function TextBlockSafe(props) {
			return createElement(XBoundary,
				{ fallback: createElement(Markdownish, { text: props.text }) },
				createElement(TextBlock, props));
		}
		// Whole-assistant fuse: if scaffolding above the block loop throws
		// (hooks, turn-data, image loader...), fall back to the original prose
		// of every text/reasoning block so the answer never vanishes.
		function AssistantViewSafe(props) {
			var fb = null;
			try {
				var node = props && props.node;
				var blocks = node && node.data && Array.isArray(node.data.blocks) ? node.data.blocks : [];
				var parts = originalBlocksText(blocks);
				if (parts.length) fb = createElement("div", null, createElement(Markdownish, { text: parts.join("\n\n") }));
			} catch (e) { fb = null; }
			return createElement(XBoundary, { fallback: fb }, createElement(AssistantView, props));
		}
		...
		function XDockTodo(props) {
			...
			return createElement("section", { className: "xl-dock", "data-testid": "todo-panel", "aria-label": title },
				createElement("div", { className: "xl-dock-body" },
					...
					open ? createElement("ul", { className: "xl-dock-list" }, list.map(function (it, i) {
						// Two todos can carry the same text (or none at all): the index keeps
						// the keys unique, which is what React needs from a key.
						return createElement(XDockTodoItem, { key: (it.content || "item") + "#" + i, item: it });
					})) : null));
		}
		...
		function StatusBlock() {
			...
			// `mem` is a plain Map, so clearing it changes nothing React can see: the
			// 缓存条目 counter kept the old number until some other state moved.
			var tickPair = useState(0);
			var bumpTick = tickPair[1];
			// "当前使用" answers the question the block exists for: which provider and
			// model the last successful translation actually went through.
			var inUse = stats.lastProvider
				? ((t.labels[stats.lastProvider] || stats.lastProvider) + (stats.lastModel ? " · " + stats.lastModel : ""))
				: t.providerDesc;
			return createElement("div", null,
				// The clear button rides the "当前使用" row, pushed to that row's end:
				// one row per idea (what is in use, then the counters), no stray rows.
				createElement("div", { className: "xl-kv" },
					createElement("b", null, t.currentInUse),
					createElement("span", { className: "xl-kv-value" }, inUse),
					createElement("button", {
						className: "xl-btn", type: "button",
						style: { marginLeft: "auto", flex: "0 0 auto" },
						onClick: function () { clearCache(); bumpTick(function (n) { return n + 1; }); }
					}, t.clearCache)),
				createElement("div", { className: "xl-kv" },
					createElement("b", null, t.status),
					createElement("span", null, stats.ok + " / " + stats.fail),
					createElement("span", { className: "xl-kv-sep" }, "·"),
					createElement("b", null, t.cacheEntries),
					createElement("span", null, String(mem.size))),
				stats.lastError ? createElement("div", { className: "xl-err" }, t.lastError + ": " + stats.lastError) : null);
		}

		// Shared label for a provider entry: the built-in openai is the local
		// Ollama ("本地部署模型（Ollama）"); any OTHER openai-type provider
		// (custom gateway or DSH-discovered) is "OpenAI-compatible" — labels are
		// keyed by provider id (google/bing/openai), so t.labels[type] would
		// mislabel every openai-type DSH/custom provider as the local model.
		function providerLabel(id, p, t) {
			if (id === "openai" && (!p || p.source !== "dsh")) return t.labels.openai;
			if (p && p.type === "openai") return t.providerOpenAICompat;
			return p && t.labels[p.type] ? t.labels[p.type] : p ? p.type : id;
		}

		function createProviderRow(id, chain, provs, cfg, t, actions) {
			var p = provs[id] || {};
			// Built-ins: google/bing (free endpoints) and openai (the local
			// Ollama provider). They live in DEFAULT_CONFIG, survive full
			// replaces on the host, and can only be toggled or removed from
			// the chain — never deleted.
			var isBuiltin = id === "google" || id === "bing" || id === "openai";
			var isDsh = p.source === "dsh";
			// Built-in openai → local Ollama label (t.labels.openai);
			// custom/DSH openai-type → generic "OpenAI-compatible" (m10);
			// everything else → t.labels[p.type] or the raw type string.
			var label = providerLabel(id, p, t);
			// Checkbox semantics: this list IS the delivery order. A DSH provider
			// is activated by chain membership (see runChainFor), so its checkbox
			// adds/removes it from the chain; every other provider keeps an
			// `enabled` flag the checkbox toggles (disabled ones are skipped).
			var inChain = chain.indexOf(id) >= 0;
			var onTitle = isDsh
				? (inChain ? t.providerRemoveFromChain : t.providerAddToChain)
				: (p.enabled !== false ? t.providerOn : t.providerOff);
			var isChecked = isDsh ? inChain : (p.enabled !== false);
			return createElement("div", {
				key: id, className: "xl-provider-row", draggable: "true",
				"data-id": id,
				onDragStart: function (e) { e.dataTransfer.setData("text/plain", id); e.dataTransfer.effectAllowed = "move"; },
				onDragOver: function (e) { e.preventDefault(); e.dataTransfer.dropEffect = "move"; },
				onDrop: function (e) { e.preventDefault(); var from = e.dataTransfer.getData("text/plain"); if (from) actions.reorder(from, id); }
			},
				createElement("span", { className: "xl-provider-type" }, label),
				// Keep the name cell on every row: it is the flex filler that lines the
				// control cluster (checkbox / arrows / test) up across rows, and the id
				// text only shows where the id is the provider's own name.
				createElement("span", { className: "xl-provider-name" }, isBuiltin ? "" : id),
				// The local endpoint carries its model right here: the chip opens the
				// picker inline instead of a separate "模型" block further down.
				id === "openai" ? createElement("button", {
					className: "xl-chip", type: "button", title: t.chooseModel,
					onClick: function () { actions.toggleModelMenu(id); }
				}, (p.model || t.chooseModel) + " ▾") : null,
				isDsh ? createElement("span", { className: "xl-badge" }, t.providerDsh) : null,
				p.apiKeyEnv ? createElement("span", { className: "xl-badge" }, "env:" + p.apiKeyEnv) : null,
				createElement("input", {
					type: "checkbox", checked: isChecked, title: onTitle,
					onChange: function (e) {
						if (isDsh) { if (e.target.checked) actions.addToChain(id); else actions.removeFromChain(id); }
						else actions.toggle(id, e.target.checked);
					}
				}),
				createElement("button", { className: "xl-btn", type: "button", title: t.providerUp, onClick: function () { actions.move(id, -1); } }, "↑"),
				createElement("button", { className: "xl-btn", type: "button", title: t.providerDown, onClick: function () { actions.move(id, 1); } }, "↓"),
				createElement("button", { className: "xl-btn", type: "button", onClick: function () { actions.test(id); } }, t.providerTest),
				// The built-ins (google / bing / the local Ollama entry) are
				// permanent: the checkbox is their on/off switch, so there is
				// nothing to remove from the list and nothing to delete. A custom
				// provider is deleted whole with the ×. A DSH row is managed by the
				// harness: unchecking it removes it from the chain only.
				!isBuiltin && !isDsh ? createElement("button", { className: "xl-btn", type: "button", onClick: function () { actions.edit(id); } }, t.providerEdit) : null,
				!isBuiltin && !isDsh ? createElement("button", { className: "xl-x-btn", type: "button", title: t.providerDelete, onClick: function () { actions.del(id); } }, "×") : null);
		}

		// ------------------------------------------------------------------
		// Draft state that must survive a remount.
		//
		// The shell keys list-slot rows by REGISTRATION IDENTITY (a fresh entry object
		// gets a fresh React key — ui-renderer scoped-slots entryKeyOf), and this panel
		// re-registers its settings section whenever the UI language changes so the nav
		// title follows along. A remount is the price of that: every useState inside the
		// panel is thrown away, which used to mean a half-typed provider form vanished
		// and the panel flashed "host half not loaded" while it refetched the config.
		//
		// So the state a user can be in the middle of — the open form and its fields,
		// the message, the menus, the download progress, and the last known config —
		// lives here instead, outside React. Refs (cfgRef/seqRef) deliberately do NOT:
		// they belong to the instance that owns the in-flight requests.
		// ------------------------------------------------------------------
		var draft = {};
		function useDraft(key, initial) {
			var pair = useState(key in draft ? draft[key] : initial);
			if (!(key in draft)) draft[key] = pair[0];
			var set = function (next) {
				// Read the updater's base from the draft, not from this render's value,
				// so two updates in one tick compose the way React's own do.
				var resolved = typeof next === "function" ? next(draft[key]) : next;
				draft[key] = resolved;
				pair[1](resolved);
			};
			return [pair[0], set];
		...
		function ProviderSection() {
			var s = useStore();
			var t = UI_TEXT[s.lang] || UI_TEXT.en;
			// The last known config is part of the draft: a remount paints the panel
			// immediately instead of the "host half is not loaded" notice, and the
			// effect below replaces it with the fresh answer.
			var pair = useDraft("cfg", null);
			var cfg = pair[0], setCfg = pair[1];
			var msgPair = useDraft("msg", "");
			var msg = msgPair[0], setMsg = msgPair[1];
			var modelsPair = useDraft("models", null);
			var models = modelsPair[0], setModels = modelsPair[1];
			var modelMenuPair = useDraft("modelMenu", null); var modelMenu = modelMenuPair[0], setModelMenu = modelMenuPair[1];
			var dshMenuPair = useDraft("dshMenu", false); var dshMenu = dshMenuPair[0], setDshMenu = dshMenuPair[1];
			useEffect(function () {
				var alive = true;
				var tries = 0;
				function attempt() {
			...
			var pullPair = useDraft("pull", null);
			var pull = pullPair[0], setPull = pullPair[1];
			var customPair = useDraft("customModel", "");
			var customModel = customPair[0], setCustomModel = customPair[1];
			var showPullPair = useDraft("showPull", false);
			var showPull = showPullPair[0], setShowPull = showPullPair[1];
			// Mutable ref to the latest cfg: synced every render and read by the
			// actions below so fast consecutive operations never act on a stale
			// closure copy (Important-3). Declared before the cfg===null guard so
			// the hook count/order stays stable across renders (Critical-1).
			var cfgRefPair = useState(function () { return { current: null }; });
			var cfgRef = cfgRefPair[0];
			cfgRef.current = cfg;
			// Monotonic save sequence: a stale /_xlate/config POST response must
			// not overwrite a newer optimistic state (Important-3/4).
			var seqRefPair = useState(function () { return { current: 0 }; });
			var seqRef = seqRefPair[0];
			// Add/edit inline form state. Lives here (not in a nested component,
			// and before the cfg guard) so re-renders — including the cfg=null →
			// cfg=value transition — never reset or reorder what the user types;
			// useDraft additionally keeps it across the remount a language change
			// causes (see the draft block above).
			var addingPair = useDraft("adding", false);
			var adding = addingPair[0], setAdding = addingPair[1];
			var editingIdPair = useDraft("editingId", null);
			var editingId = editingIdPair[0], setEditingId = editingIdPair[1];
			var formTypePair = useDraft("formType", "openai"); var formType = formTypePair[0], setFormType = formTypePair[1];
			var formProviderPair = useDraft("formProvider", ""); var formProvider = formProviderPair[0], setFormProvider = formProviderPair[1];
			var formProviderMenuPair = useDraft("formProviderMenu", false); var formProviderMenu = formProviderMenuPair[0], setFormProviderMenu = formProviderMenuPair[1];
			var formModelMenuPair = useDraft("formModelMenu", false); var formModelMenu = formModelMenuPair[0], setFormModelMenu = formModelMenuPair[1];
			var formBaseURLPair = useDraft("formBaseURL", ""); var formBaseURL = formBaseURLPair[0], setFormBaseURL = formBaseURLPair[1];
			var formApiKeyPair = useDraft("formApiKey", ""); var formApiKey = formApiKeyPair[0], setFormApiKey = formApiKeyPair[1];
			var formApiKeyEnvPair = useDraft("formApiKeyEnv", ""); var formApiKeyEnv = formApiKeyEnvPair[0], setFormApiKeyEnv = formApiKeyEnvPair[1];
			var formModelPair = useDraft("formModel", ""); var formModel = formModelPair[0], setFormModel = formModelPair[1];
			...
			// Optimistically merge a patch into the live cfg so consecutive
			// operations (delete A then delete B) each see the previous edit,
			// and the UI reflects changes immediately instead of after the POST
			// round-trip (Important-3). Handles the `_full` full-replace form.
			var applyLocal = function (patch) {
				var cur = cfgRef.current || {};
				var next = Object.assign({}, cur);
				if (Array.isArray(patch.chain)) next.chain = patch.chain.slice();
				if (patch.providers && typeof patch.providers === "object") {
					var pv = patch.providers;
					if (pv._full && pv._full._full === true) {
						var fm = pv._full, nprov = {}, fkeys = Object.keys(fm);
						for (var i = 0; i < fkeys.length; i++) {
							if (fkeys[i] !== "_full") nprov[fkeys[i]] = fm[fkeys[i]];
						}
						next.providers = nprov;
					} else {
						var mprov = Object.assign({}, cur.providers || {});
						var pkeys = Object.keys(pv);
						for (var k = 0; k < pkeys.length; k++) {
							mprov[pkeys[k]] = Object.assign({}, mprov[pkeys[k]] || {}, pv[pkeys[k]]);
						}
						next.providers = mprov;
					}
				}
				cfgRef.current = next;
				setCfg(next);
			};
			// Persist a validated config patch through the /_xlate/config route
			// (Task 5). Checks r.ok so a 500 (JSON error body) never replaces the
			// live config (Important-4), and drops stale responses via seqRef so a
			// slow earlier save cannot clobber a newer one (Important-3).
			var saveCfg = function (patch, okMsg) {
				...
				fetch("/_xlate/config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) })
					...
					.catch(function (e) {
						if (seq !== seqRef.current) return;
						setMsg((t.providerSaveError || t.connFail) + ": " + (e && e.message ? e.message : e));
						// Resync with the host truth so a failed save doesn't leave
						// the optimistic state permanently divergent.
						fetch("/_xlate/config").then(function (r) { return r.json(); })
							.then(function (c) { if (c && seq === seqRef.current) { cfgRef.current = c; setCfg(c); } })
							.catch(function () {});
					});
			};
			var startPull = function (m) {
				...
				// The POST only starts the job (the 1 s poll reports progress), so a
				// rejected start must surface here — otherwise the panel sits in
				// "downloading" forever and the user is never told why.
				fetch("/_xlate/model/pull", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: name }) })
					...
					.catch(function (e) {
						setPull({ model: name, status: "error", percent: 0, done: false, error: e && e.message ? e.message : "network" });
					});
			};
			var pickModel = function (m) {
				if (!m) return;
				var cur = cfgRef.current || {};
				var openai = (cur.providers && cur.providers.openai) || {};
				applyLocal({ providers: { openai: { model: m } } });
				setModelMenu(null);
				fetch("/_xlate/config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ providers: { openai: { model: m } } }) })
					.then(function (r) { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
					.then(function (c) { cfgRef.current = c; setCfg(c); })
					.catch(function (e) {
						// Do not leave the chip showing a model the host never stored.
						setMsg((t.providerSaveError || t.connFail) + ": " + (e && e.message ? e.message : e));
						fetch("/_xlate/config").then(function (r) { return r.json(); })
							.then(function (c) { if (c) { cfgRef.current = c; setCfg(c); } })
							.catch(function () {});
					});
			};
			...
			// (the local-model picker now renders inside its provider row — see modelPanel)
			var actions = {
				reorder: function (from, to) {
					var chain = cfgRef.current.chain.slice();
					var i = chain.indexOf(from), j = chain.indexOf(to);
					if (i < 0 || j < 0) return;
					chain.splice(i, 1);
					// Removing the dragged item shifts later indexes down by one
					// (Important-2: dragging down produced a wrong ordering).
					if (i < j) j--;
					chain.splice(j, 0, from);
					applyLocal({ chain: chain });
					saveCfg({ chain: chain });
				},
				...
				toggle: function (id, on) {
					var patch = { providers: {} };
					patch.providers[id] = { enabled: on };
					applyLocal(patch);
					saveCfg(patch);
				},
				toggleModelMenu: function (id) {
					setModelMenu(function (v) { return v === id ? null : id; });
				},
				addToChain: function (id) {
					var chain = (cfgRef.current.chain || []).slice();
					if (chain.indexOf(id) >= 0) return;
					chain.push(id);
					applyLocal({ chain: chain });
					saveCfg({ chain: chain });
				},
				removeFromChain: function (id) {
					var chain = (cfgRef.current.chain || []).filter(function (x) { return x !== id; });
					applyLocal({ chain: chain });
					saveCfg({ chain: chain });
				},
				test: function (id) {
					...
					fetch("/_xlate/translate", { method: "POST", headers: { "Content-Type": "application/json" },
						body: JSON.stringify({ text: "Hello world, connection test. [" + Date.now().toString(36) + "]", target: s.lang, provider: id }) })
						.then(function (r) { return r.json(); })
						.then(function (j) { setMsg(j && j.ok ? t.connOk + " ✓ (" + providerLabel(id, (cfgRef.current.providers || {})[id] || {}, t) + (j.model ? " · " + j.model : "") + ")" : (t.connFail + ": " + (j.error || "unknown"))); })
						.catch(function (e) { setMsg(t.connFail + ": " + (e && e.message ? e.message : e)); });
				},
				edit: function (id) {
					var p = cfgRef.current.providers && cfgRef.current.providers[id];
					if (!p) return;
					setAdding(false);
					setEditingId(id);
					setFormType(p.type || "openai");
					setFormBaseURL(p.baseURL || "");
					setFormApiKey(p.apiKey || "");
					setFormApiKeyEnv(p.apiKeyEnv || "");
					var presetForEntry = null;
					for (var pi = 0; pi < PROVIDER_PRESETS.length; pi++) {
						if (PROVIDER_PRESETS[pi].baseURL === (p.baseURL || "")) { presetForEntry = PROVIDER_PRESETS[pi]; break; }
					}
					setFormProvider(presetForEntry ? presetForEntry.name : id);
					setFormProviderMenu(false);
					setFormModelMenu(false);
					setFormModel(p.model || "");
				},
				del: function (id) {
					var chain = cfgRef.current.chain.filter(function (x) { return x !== id; });
					var providers = Object.assign({}, cfgRef.current.providers);
					delete providers[id];
					// Full-replace convention: the marker `_full: true` lives
					// INSIDE the map value the client sends as `providers._full`.
					var fullMap = { _full: true };
					var keys = Object.keys(providers);
					for (var i = 0; i < keys.length; i++) {
						fullMap[keys[i]] = providers[keys[i]];
					}
					applyLocal({ providers: { _full: fullMap }, chain: chain });
					saveCfg({ providers: { _full: fullMap }, chain: chain }, t.providerDeleted);
				},
			};

			// Provider names are a suggestion list, not a constraint: typing any name
			// keeps the endpoint fields under the user's control, and a name that
			// matches a known provider fills them in.
			var presetByName = function (name) {
				var want = String(name || "").trim().toLowerCase();
				for (var i = 0; i < PROVIDER_PRESETS.length; i++) {
					if (PROVIDER_PRESETS[i].name.toLowerCase() === want) return PROVIDER_PRESETS[i];
				}
				return null;
			};
			// The model list belongs to one provider: with none chosen there is nothing
			// to offer, and the model button stays disabled rather than dumping every
			// model this endpoint type knows about.
			var modelsForProvider = function (name) {
				var pp = presetByName(name);
				return pp ? pp.models : [];
			};
			var onProviderChange = function (value) {
				setFormProvider(value);
				var pp = presetByName(value);
				if (!pp) return;
				setFormType(pp.type);
				setFormBaseURL(pp.baseURL);
				setFormApiKeyEnv(pp.env);
				// One auth source at a time: a preset that names an env var takes over
				// from a literal key typed a moment ago (apiKeyEnv wins host-side, so
				// keeping both would silently ignore the key).
				if (pp.env) setFormApiKey("");
				if (pp.models.length && pp.models.indexOf(formModel.trim()) < 0) setFormModel(pp.models[0]);
			};
			var submitForm = function () {
				var model = formModel.trim();
				if (!model || !formBaseURL.trim()) return;
				// The model name IS this provider's name; a second provider on the same
				// model gets a numeric suffix so ids stay unique.
				var id = editingId || model;
				var nth = 2;
				while (!editingId && cfgRef.current.providers && cfgRef.current.providers[id]) {
					id = model + "-" + nth;
					nth++;
				}
				var existing = cfgRef.current.providers && cfgRef.current.providers[id];
				var providers = {};
				providers[id] = {
					type: formType,
					baseURL: formBaseURL.trim(),
					apiKey: formApiKey.trim() === "" ? null : formApiKey.trim(),
					apiKeyEnv: formApiKeyEnv.trim() === "" ? null : formApiKeyEnv.trim(),
					model: formModel.trim(),
					enabled: existing ? existing.enabled !== false : true,
				};
				var chain = cfgRef.current.chain.slice();
				if (!editingId && chain.indexOf(id) < 0) chain.push(id);
				applyLocal({ providers: providers, chain: chain });
				saveCfg({ providers: providers, chain: chain }, t.providerSaved);
				setAdding(false);
				setEditingId(null);
			};
			...
			var chain = Array.isArray(cfg.chain) ? cfg.chain : [];
			var provs = cfg.providers || {};

			// ---- inherit the harness's own providers ------------------------
			// DSH already knows its endpoints (llm-pi-ai.providers in the harness
			// settings.yaml) and where their keys live (.credentials.yaml). The host
			// exposes them as read-only `source: 'dsh'` entries and re-reads them on
			// demand, so the one-click path is: rescan, then add — never retype a base
			// URL or a key per provider.
			var dshAll = Object.keys(provs).filter(function (id) { return provs[id] && provs[id].source === "dsh"; });
			var dshAvailable = dshAll.filter(function (id) { return chain.indexOf(id) < 0; });
			// The host already merges the harness's providers into the config it serves,
			// so they show up in the "available providers" list without being asked for.
			// What the button adds is the RE-READ: editing ~/.dsh/settings.yaml (or
			// fixing the home the host looks at) otherwise needs a web restart.
			var rescanDsh = function () {
				setMsg(t.dshRescanning);
				fetch("/_xlate/dsh-scan").then(function (r) { return r.json(); })
					.then(function (c) {
						if (!c || !c.providers) throw new Error("bad scan response");
						cfgRef.current = c;
						setCfg(c);
						var d = c.dshScan || {};
						var found = (d.providers || []).length;
						setMsg(found
							? t.dshRescanDone + " · " + found
							: t.dshImportEmpty + (d.home ? " · " + d.home + "/settings.yaml" : ""));
					})
					.catch(function (e) { setMsg(t.connFail + ": " + (e && e.message ? e.message : e)); });
			};
			var addAllDsh = function () {
				var next = chain.slice();
				dshAvailable.forEach(function (id) { if (next.indexOf(id) < 0) next.push(id); });
				applyLocal({ chain: next });
				saveCfg({ chain: next });
			};

			// Local-model menu: opened by the chip on the endpoint's own row and
			// rendered inline directly under it, so nothing floats, clips or has to be
			// repositioned. The download controls live here too — they belong to the
			// model list they extend, not to a separate block.
			var openaiCfg = provs.openai || {};
			var openaiBase = openaiCfg.baseURL || "http://localhost:11434/v1";
			var isOllama = openaiBase.indexOf("localhost") >= 0 || openaiBase.indexOf("127.0.0.1") >= 0;
			var modelPanel = null;
			if (modelMenu === "openai") {
				// The download state belongs to the panel, not to the "a model is
				// installed" case: with an empty list the picker shows nothing but the
				// download buttons, and that is exactly when progress and failures have
				// to be visible (Ollama down used to be a silent black hole).
				var pullArea = null;
				if (pull && !pull.done && !pull.error) {
					var stText = t.downloading;
					// Ollama reports layer downloads as status "pulling sha256:…",
					// not "downloading" — show the percentage whenever bytes move.
					...
					pullArea = createElement("div", null,
						...
						downloadButtons());
				} else if (showPull && isOllama) {
					pullArea = downloadButtons();
				}
				var modelList = null;
				if (models === null) {
					modelList = createElement("div", { className: "xl-note" }, t.loadingModels);
				} else if (models.length === 0) {
					modelList = createElement("div", null,
						createElement("div", { className: "xl-note" }, t.noLocalModel + " · " + t.installPrompt),
						pullArea || downloadButtons());
				} else {
					modelList = createElement("div", null,
						createElement("div", { className: "xl-model-menu" }, models.map(function (m) {
							var active = (openaiCfg.model || "") === m;
							return createElement("button", {
								key: m, type: "button",
								className: "xl-model-item" + (active ? " xl-model-item-active" : ""),
								onClick: function () { pickModel(m); }
							}, m);
						})),
						createElement("div", { style: { display: "flex", gap: "6px" } },
							isOllama ? createElement("button", {
								className: "xl-btn", type: "button",
								onClick: function () { setShowPull(function (v) { return !v; }); }
							}, t.downloadModel + " …") : null,
							createElement("button", {
								className: "xl-btn", type: "button",
								onClick: function () { setModelMenu(null); }
							}, t.providerCancel)),
						pullArea);
				}
				modelPanel = createElement("div", { className: "xl-model-panel", key: "xl-model-panel" }, modelList);
			}
			var rows = [];
			chain.forEach(function (id) {
				rows.push(createProviderRow(id, chain, provs, cfg, t, actions));
				if (modelMenu === id) rows.push(modelPanel);
			});

			var formArea = null;
			if (adding || editingId) {
				formArea = createElement("div", { className: "xl-add-form" },
				// 类型 first: it decides which providers the next row can offer.
				createElement("label", { className: "xl-form-row" },
					createElement("span", { className: "xl-form-label" }, t.providerType),
					createElement("select", {
						className: "xl-sel", style: { flex: "1 1 auto" }, value: formType,
						onChange: function (e) { setFormType(e.target.value); setFormProvider(""); }
					},
						createElement("option", { value: "openai" }, t.providerOpenAICompat),
						createElement("option", { value: "anthropic" }, "Anthropic"))),
				// 提供方 next to the model name: pick a known one to prefill, or keep 自己填 and
				// type everything. The model name doubles as this provider's name.
				createElement("label", { className: "xl-form-row" },
				createElement("span", { className: "xl-form-label" }, t.providerPreset),
				createElement("span", { className: "xl-form-field", style: { flex: "0 0 150px" } },
					createElement("input", {
						className: "xl-cfg", placeholder: t.presetCustom, value: formProvider,
						onChange: function (e) { onProviderChange(e.target.value); }
					}),
					createElement("button", {
						className: "xl-caret", type: "button", title: t.providerPreset,
						onClick: function () { setFormProviderMenu(function (v) { return !v; }); }
					}, "▾"),
					// A boxed menu under the field, not a cloud of chips and not the browser's
					// own suggestion list (which filters itself by the current value).
					formProviderMenu ? createElement("div", { className: "xl-menu" },
						PROVIDER_PRESETS.map(function (pp) {
							if (pp.type !== formType) return null;
							return createElement("button", {
								key: pp.name, type: "button",
								className: "xl-menu-item" + (pp.name === formProvider ? " xl-menu-item-active" : ""),
								onClick: function () { onProviderChange(pp.name); setFormProviderMenu(false); }
							}, pp.name);
						}),
						createElement("div", { className: "xl-menu-note" }, t.providerHint)) : null),
				createElement("span", { className: "xl-form-field", style: { flex: "1 1 auto" } },
					createElement("input", {
						className: "xl-cfg", placeholder: t.providerModel, value: formModel,
						onChange: function (e) { setFormModel(e.target.value); }
					}),
					createElement("button", {
						className: "xl-caret", type: "button", title: t.providerModel,
						disabled: !presetByName(formProvider),
						onClick: function () { if (presetByName(formProvider)) setFormModelMenu(function (v) { return !v; }); }
					}, "▾"),
					// Only reachable once a provider is known, so the list is always that one
					// provider's models.
					formModelMenu && presetByName(formProvider) ? createElement("div", { className: "xl-menu" },
						modelsForProvider(formProvider).map(function (m) {
							return createElement("button", {
								key: m, type: "button",
								className: "xl-menu-item" + (m === formModel ? " xl-menu-item-active" : ""),
								onClick: function () { setFormModel(m); setFormModelMenu(false); }
							}, m);
						}),
						createElement("div", { className: "xl-menu-note" }, t.modelHint)) : null)),
				createElement("label", { className: "xl-form-row" },
					createElement("span", { className: "xl-form-label" }, t.providerBaseURL),
					createElement("input", {
						className: "xl-cfg", placeholder: "https://api.example.com/v1", value: formBaseURL,
						onChange: function (e) { setFormBaseURL(e.target.value); }
					})),
				createElement("label", { className: "xl-form-row" },
					createElement("span", { className: "xl-form-label" }, t.providerApiKey),
					// No env badge here: the apiKeyEnv a preset fills in is an
					// implementation detail of that endpoint, and the saved row shows an
					// env:NAME badge anyway. Typing a key clears the env name (below), so
					// the key the user can see is always the one that wins.
					createElement("input", {
						className: "xl-cfg", placeholder: t.providerApiKey, value: formApiKey,
						onChange: function (e) {
							var v = e.target.value;
							setFormApiKey(v);
							if (v.trim() !== "") setFormApiKeyEnv("");
						}
					})),
				createElement("div", { style: { display: "flex", gap: "6px" } },
					createElement("button", { className: "xl-btn", type: "button", onClick: submitForm }, t.providerSave),
					createElement("button", { className: "xl-btn", type: "button", onClick: cancelForm }, t.providerCancel)));
			}

			// Providers present in cfg.providers but NOT in the chain: DSH
			// auto-discovered endpoints and custom providers that were added
			// without a chain reference. Each gets an "add to chain" button so
			// DSH providers can actually be activated (chain membership IS the
			// activation for source:'dsh' — see runChainFor).
			var unused = Object.keys(provs).filter(function (id) {
				return chain.indexOf(id) < 0;
			});
			var unusedDsh = unused.filter(function (id) { return provs[id] && provs[id].source === "dsh"; });
			var unusedArea = null;
			if (unused.length > 0) {
				// The heading names what the rows are — in practice the DSH entries the
				// host discovered; the generic wording stays for a custom provider that
				// was saved without a chain reference.
				var allDsh = unusedDsh.length === unused.length;
				unusedArea = createElement("div", { className: "xl-unused", style: { marginTop: "8px" } },
					// The heading row carries the bulk action: the rows are right here, so
					// this is the one place that lists them (the old separate "inherit"
					// menu listed the same providers a second time).
					createElement("div", { style: { display: "flex", alignItems: "center", gap: "6px" } },
						createElement("div", { className: "xl-note", style: { fontWeight: 600, flex: "1 1 auto" } },
							allDsh ? t.providerDshList : t.providerUnused),
						unusedDsh.length > 1 ? createElement("button", {
							className: "xl-btn", type: "button", onClick: addAllDsh
						}, t.dshImportAll) : null),
					unused.map(function (id) {
						var up = provs[id] || {};
						var isDsh = up.source === "dsh";
						return createElement("div", { key: id, className: "xl-provider-row" },
							createElement("span", { className: "xl-provider-type" }, providerLabel(id, up, t)),
							createElement("span", { className: "xl-provider-name" }, id),
							isDsh ? createElement("span", { className: "xl-badge" }, t.providerDsh) : null,
							// A bare "+" with the wording in the tooltip: one narrow control
							// at the end of the row, same as the ↑↓/测试 cluster above.
							createElement("button", {
								className: "xl-btn xl-add-btn", type: "button",
								title: t.providerAddToChain, "aria-label": t.providerAddToChain,
								onClick: function () { actions.addToChain(id); }
							}, "+"));
					}));
			}

			return createElement("div", null,
				createElement("div", { className: "xl-note", style: { marginBottom: "2px", fontWeight: 600 } }, t.providerDrag),
				rows,
				formArea,
				adding || editingId ? null
					: createElement("button", { className: "xl-btn", type: "button", onClick: startAdd, style: { marginTop: "4px" } },
						"+ " + t.providerAdd),
				adding || editingId ? null
					: createElement("button", { className: "xl-btn", type: "button", onClick: rescanDsh, style: { marginTop: "4px", marginLeft: "6px" } },
						"↻ " + t.dshImport),
				unusedArea,
				msg ? createElement("div", { className: "xl-note", style: { marginTop: "6px" } }, msg) : null);
		}

		// A small open book, drawn inline: the plugin must not depend on a platform
		// glyph (or on a DSH release that ships the settings.section.icon seat) for
		// users to recognize the section.
		var NAV_GLYPH_SVG = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">'
			+ '<path d="M8 3.3C6.9 2.5 5.5 2.1 4 2.1H2.6v10.2H4c1.5 0 2.9.4 4 1.2 1.1-.8 2.5-1.2 4-1.2h1.4V2.1H12c-1.5 0-2.9.4-4 1.2Z" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round"/>'
			+ '<path d="M8 3.3v10.2" stroke="currentColor" stroke-width="1.2"/></svg>';

		// Swap this plugin's own glyph into its settings nav cell. The cell is shell
		// chrome, so the match is deliberately narrow: a button inside a <nav> whose
		// last element carries exactly the section title registered above. The
		// marker attribute makes it idempotent (React re-renders must not stack
		// replacements), and a missing glyph degrades to the shell's own.
		//
		// The shell's own glyph is HIDDEN, never removed: React keeps a reference to
		// that node, and taking it out of the tree behind React's back means React
		// can later patch or remove a detached node (NotFoundError on removeChild).
		// A sibling in the same slot looks identical and leaves React's tree intact.
		function shellNavIcon(btn) {
			// The SHELL's svg, never ours: once the holder is in place it comes first
			// in document order, so a plain querySelector('svg') returns the book — and
			// the next pass then hides the plugin's own glyph. That is exactly how the
			// nav icon disappeared after the first pass (fixed in 1.2.1).
			var kids = btn.children || [];
			for (var i = 0; i < kids.length; i++) {
				var tag = (kids[i].tagName || "").toLowerCase();
				if (tag === "svg" && !kids[i].hasAttribute("data-xl-nav-glyph-svg")) return kids[i];
			}
			var nested = btn.querySelectorAll ? btn.querySelectorAll("svg") : [];
			for (var j = 0; j < nested.length; j++) {
				if (!nested[j].hasAttribute("data-xl-nav-glyph-svg")) return nested[j];
			}
			return null;
		}

		function decorateSettingsGlyph() {
			if (typeof document === "undefined") return;
			var st = UI_TEXT[store.lang] || UI_TEXT.en;
			var title = (st.sectionTitle || "").trim();
			if (!title) return;
			...
			for (var i = 0; i < navs.length; i++) {
				...
				for (var j = 0; j < buttons.length; j++) {
					...
					var svg = shellNavIcon(btn);
					if (!svg || !svg.parentNode) continue;
					if (!btn.querySelector('[data-xl-nav-glyph="1"]')) {
						var holder = document.createElement("span");
						holder.setAttribute("data-xl-nav-glyph", "1");
						// Carry the shell glyph's class over, so shell CSS that targets
						// the icon still applies to the shape standing in for it.
						var svgClass = svg.getAttribute ? svg.getAttribute("class") : null;
						if (svgClass) holder.setAttribute("class", svgClass);
						holder.style.display = "inline-flex";
						holder.innerHTML = NAV_GLYPH_SVG;
						var inner = holder.querySelector ? holder.querySelector("svg") : null;
						if (inner) inner.setAttribute("data-xl-nav-glyph-svg", "1");
						svg.parentNode.insertBefore(holder, svg);
					}
					// Self-healing: the shell may re-render its icon, and the decoration
					// must not end up showing two glyphs side by side.
					if (!svg.getAttribute || !svg.getAttribute("data-xl-nav-glyph-hidden")) {
						if (svg.setAttribute) svg.setAttribute("data-xl-nav-glyph-hidden", svg.style ? (svg.style.display || "") : "");
		...
		// Undo the DOM edit when the plugin is disposed: without this the shell keeps
		// showing this plugin's glyph after the plugin is gone.
		function undecorateSettingsGlyph() {
			if (typeof document === "undefined") return;
			var holders = document.querySelectorAll('[data-xl-nav-glyph="1"]');
			for (var i = 0; i < holders.length; i++) {
				var h = holders[i];
				if (h.parentNode) h.parentNode.removeChild(h);
			}
			var hidden = document.querySelectorAll("[data-xl-nav-glyph-hidden]");
			for (var j = 0; j < hidden.length; j++) {
				var svg = hidden[j];
				if (svg.style) svg.style.display = svg.getAttribute("data-xl-nav-glyph-hidden") || "";
				if (svg.removeAttribute) svg.removeAttribute("data-xl-nav-glyph-hidden");
			}
		}
		...
		function SettingsPanel() {
			var s = useStore();
			var t = UI_TEXT[s.lang] || UI_TEXT.en;
			// The version comes from the installed package (host route) instead of a
			// constant somebody has to remember to bump.
			var aboutPair = useState(null);
			var about = aboutPair[0], setAbout = aboutPair[1];
			useEffect(function () {
				var alive = true;
				fetch("/_xlate/version").then(function (r) { return r.json(); })
					.then(function (j) { if (alive && j && j.ok) setAbout(j); })
					.catch(function () {});
				return function () { alive = false; };
			}, []);
			return createElement("div", { className: "xl-panel" },
				...
				createElement("hr", { className: "xl-hr" }),
				createElement("div", { className: "xl-about" },
					createElement("span", null, ((about && about.name) || PLUGIN_NAME) + " v" + ((about && about.version) || PLUGIN_VERSION)),
					createElement("a", {
						className: "xl-btn", style: { textDecoration: "none" },
						href: (about && about.repo) || PLUGIN_REPO,
		...
		function apply(ctx) {
				...
				return ctx.slots.inject("conversation.chat.node", function () {
					try {
						var d = ctx.slots.register(
							...
							function (props) {
								return createElement(AssistantViewSafe, props);
							});
			...
			// The nav glyph seat is dispatched by section id and carries no text, so
			// it is registered once instead of following the language changes above.
			// Absent seat (older DSH) or absent primitives: nothing happens and the
			// shell renders its own glyph.
			recordSlot("settingsIcon", function () {
				return ctx.slots.inject("settings.section.icon", function () {
					if (!IconThink) return undefined;
					return ctx.slots.register(
						{ name: "settings.section.icon", key: "dsh-think-translate" },
						function () { return createElement(IconThink, { size: 16 }); });
				});
			});

			// Fallback for platforms without that seat (every released DSH today):
			// decorate the row once it is on screen. The observer only reacts to
			// mutations that touch (<nav>) content or add a nav, so the steady-state
			// cost while messages stream is a few cheap record checks, and the scan
			// itself is debounced.
			recordSlot("settingsGlyph", function () {
				// documentElement rather than body: the bundle is loaded "immediately",
				// so <body> may not exist yet, while the root element always does.
				if (typeof document === "undefined" || !document.documentElement ||
					typeof MutationObserver !== "function") return undefined;
				var pending = null;
				var schedule = function () {
					if (pending !== null) return;
					pending = setTimeout(function () { pending = null; decorateSettingsGlyph(); }, 150);
				};
				var touchesNav = function (records) {
					for (var i = 0; i < records.length; i++) {
						var r = records[i];
						if (r.target && r.target.closest && r.target.closest("nav")) return true;
						var added = r.addedNodes || [];
						for (var k = 0; k < added.length; k++) {
							var n = added[k];
							if (n && n.nodeType === 1 && (n.tagName === "NAV" || (n.querySelector && n.querySelector("nav")))) return true;
						}
					}
					return false;
				};
				var observer = new MutationObserver(function (records) { if (touchesNav(records)) schedule(); });
				observer.observe(document.documentElement, { childList: true, subtree: true });
				schedule();
				var dispose = function () {
					if (pending !== null) clearTimeout(pending);
					observer.disconnect();
					undecorateSettingsGlyph();
				};
				// recordSlot's return value is dropped, so hand the cleanup to Cordis
				// as well: that is what runs when the plugin is disposed. Best effort —
				// a runtime without ctx.effect must not break apply().
				try { if (typeof ctx.effect === "function") ctx.effect(function () { return dispose; }); } catch (e) {}
				return dispose;
			});
		}
